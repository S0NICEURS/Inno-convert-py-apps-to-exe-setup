...
[Icons]
Name: "{{group}}\\{os.path.basename(source_folder)}"; Filename: "{{app}}\\{main_script}"; WorkingDir: "{{app}}"; IconFilename: "{{app}}\\{ico_file}"
Name: "{{commondesktop}}\\{os.path.basename(source_folder)}"; Filename: "{{app}}\\{main_script}"; WorkingDir: "{{app}}"; IconFilename: "{{app}}\\{ico_file}"
...
def save_and_convert():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            inno_script = create_inno_script(source_folder, output_dir, 'icon.ico', main_script, custom_script, install_dir, background_temp)
            ...
            if not os.path.isfile(iscc_path):
                logger.info(f"Inno Setup compiler not found at {iscc_path}. Attempting to install Inno Setup...")
                install_inno()
                return
