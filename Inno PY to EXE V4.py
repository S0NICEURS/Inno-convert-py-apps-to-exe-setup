...
def create_inno_script(source_folder, output_dir, ico_file, main_script, custom_script, install_dir):
    return f"""
[Setup]
...
def save_and_convert():
    ...
    install_dir = install_dir_entry.get() or os.path.join(os.getenv('USERPROFILE'), 'Documents')
    ...
    try:
        ...
        # Convertir le fichier PNG en ICO et le stocker dans le dossier source
        ico_file_output = os.path.join(source_folder, 'icon.ico')
        img = Image.open(icon_file)
        img.save(ico_file_output, format='ICO', sizes=[(256, 256)])
        ...
        inno_script = create_inno_script(source_folder, output_dir, ico_file_output, main_script, custom_script, install_dir)
        script_path = os.path.join(output_dir, 'installer.iss')
...
install_dir_entry.insert(0, os.path.join(os.getenv('USERPROFILE'), 'Documents'))
